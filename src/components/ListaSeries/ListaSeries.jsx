...
import CardGaleriaSerie from './CardGaleriaSerie';
...
const ListaSeries = (props) => {
    ...
    const CargarGeneros = (datos) => {
        let lista = document.querySelector("#lista-generos-series");
    ...
    const CargarMasPeliculas = async () => {
        try {
            ...
            if (filtrarRef.current == false) {
                ...
                await fetch(`https://api.themoviedb.org/3/discover/tv?api_key=${api_key}&page=${page}&language=${language}`)
    ...
    const CargarMasPeliculasGenero = async () => {
        ...
        if (filtrarRef.current == true) {
            ...
            await fetch(`https://api.themoviedb.org/3/discover/tv?api_key=${api_key}&page=${page}&with_genres=${genero}&${language}`)
                .then(response => response.json())
                .then(data => {
                    listaMovies = data.results;
            ...
            <section>
                ...
                <article className='contenedor-filtrado'>
                    ...
                    <button className="btn btn-filtrado dropdown-toggle" type="button" id="dropdownMenuGeneros" data-bs-toggle="dropdown" data-bs-target="#lista-generos" aria-expanded="false">
                        Generos</button>
                    <ul id='lista-generos-series' aria-labelledby='dropdownMenuGeneros' className='dropdown-menu contenedor-generos'></ul>
                </article>
                <article className='lista-galeria'>
                    {
                        movie.map(pelicula => {
                            return <CardGaleriaSerie SeleccionarCard={SeleccionarCard} openModal={ModalMostrar} id={pelicula.id} key={pelicula.id} pelicula={pelicula}></CardGaleriaSerie>
                        })
                    }
                </article>
                <article>
